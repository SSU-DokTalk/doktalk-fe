...
function Profile({
    ...
    setCurrentTab?: Dispatch<
        SetStateAction<MyTabsCandidate | UserTabsCandidate>
    >
}) {
    const [tabs, setTabs] =
    ...
    useEffect(() => {
        if (userProfile.id != 0) {
            axios
                .get(`/api/user/is-following/${userProfile.id}`)
                .then((res) => {
            ...
            <div className='profile-container mx-8! md:w-3/5! md:mx-auto!'>
                    ...
                    <div className='profile-image-container'>
                        ...
                        <div className='button-container'>
                            {user.id != 0 && userProfile.id == user.id ? (
                                ...
                                >
                                    {t(
                                        'component.section.profile.button.edit-profile'
                                    )}
                                </button>
                            ) : (
                                <button
                                ...
                                >
                                    {isFollowing
                                        ? t(
                                              'component.section.profile.button.unfollow'
                                          )
                                        : t(
                                              'component.section.profile.button.follow'
                                          )}
                                </button>
                            )}
                        </div>
                ...
                <div className='lower-container'>
                    ...
                    <div className='follow-container'>
                        ...
                        >
                            {t(
                                'component.section.profile.text.follower-prefix'
                            )}
                            {userProfile.follower_num}
                            {t(
                                'component.section.profile.text.follower-postfix'
                            )}
                        </div>
                        <div
                            className='following'
                        ...
                        >
                            {t(
                                'component.section.profile.text.following-prefix'
                            )}
                            {userProfile.following_num}
                            {t(
                                'component.section.profile.text.following-postfix'
                            )}
                        </div>
                    </div>
                    <div className='introduction-container'>
                        <pre className='introduction'>
                            {userProfile.introduction ??
                                t(
                                    'component.section.profile.text.no-introduction'
                                )}
                        </pre>
                    </div>
                </div>
